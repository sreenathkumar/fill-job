...
function EditJobProfile() {
   ...
   const [previewProSignature, setPreviewProSignature] = useState<string | undefined>();
   const [photoUploadError, setPhotoUploadError] = useState<string | undefined>();
   const [signatureUploadError, setSignatureUploadError] = useState<string | undefined>();
   ...
   const handleSubmit = (event: React.MouseEvent) => {
      ...
      if (textForm && fileForm) {
         handleFileSubmit({ preventDefault: () => { }, currentTarget: fileForm } as React.FormEvent<HTMLFormElement>)
         handleTextSubmit({ preventDefault: () => { }, currentTarget: textForm } as React.FormEvent<HTMLFormElement>)
      }
   };
   ...
   const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      console.log(await checkValidImage(selectedFile?.[0], 300, 300));
      if (name === 'photo') {
   ...
   return (
      <Box display={'flex'} flexDirection={'column'}>
         <Box component='form' onSubmit={handleTextSubmit} id='text-form' sx={{ display: 'flex', flexDirection: 'column', gap: '2.5rem' }} >
         ...
         <Box component='form' onSubmit={handleFileSubmit} id='file-form' my={'2.5rem'} >
            <Grid container padding={'1.5rem'} borderRadius={'10px'} rowGap={'1rem'} elevation={3} component={Paper} square>
               <Box sx={{ display: 'flex', gap: '1rem' }} width={'100%'}>
                  <Avatar variant="square" sx={{ width: '100px', height: '100px' }}>
                     <img src={previewProPhoto} alt="" width={'100%'} height={'100%'} />
                  </Avatar>
                  <Button
                     component="label"
                     tabIndex={-1}
                  ...
                  >
                     ...
                     {photoUploadError && <Alert severity="error">{photoUploadError}</Alert>}
                     <VisuallyHiddenInput type="file" name='photo' onChange={handleFileUpload} />
                  </Button>
               </Box>
               <Box sx={{ display: 'flex', gap: '1rem' }} width={'100%'}>
                  <Avatar variant="square" sx={{ width: '300px', height: '80px' }}>
                     <img src={previewProSignature} alt="" width={'100%'} height={'100%'} />
                  </Avatar>
                  <Button
                     component="label"
                     tabIndex={-1}
                  ...
                  >
                     ...
                     {signatureUploadError && <Alert severity="error">{signatureUploadError}</Alert>}
                     <VisuallyHiddenInput type="file" name='signature' onChange={handleFileUpload} />
                  </Button>
               </Box>
